...
test('Symbol size', (t) => {
	t.throws(() => { Utils.getSymbolSize(); }, 'Should throw if version is undefined');
	t.throws(() => { Utils.getSymbolSize(0); }, 'Should throw if version is not in range');
	t.throws(() => { Utils.getSymbolSize(41); }, 'Should throw if version is not in range');
...
test('Set/Get SJIS function', (t) => {
	t.throw(() => { Utils.setToSJISFunction(); },
