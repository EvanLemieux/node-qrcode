...
test('Version capacity', (t) => {
	t.throws(() => { Version.getCapacity(); }, 'Should throw if version is undefined');
	t.throws(() => { Version.getCapacity(''); }, 'Should throw if version is not a number');
	t.throws(() => { Version.getCapacity(0); }, 'Should throw if version is not in range');
	t.throws(() => { Version.getCapacity(41); }, 'Should throw if version is not in range');
...
test('Version encoded info', (t) => {
	...
	for (v = 0; v < 7; v++) {
		t.throws(() => { Version.getEncodedBits(v); },
