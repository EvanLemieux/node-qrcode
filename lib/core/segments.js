import { ALPHANUMERIC as _ALPHANUMERIC, BYTE as _BYTE, KANJI as _KANJI, NUMERIC as _NUMERIC, BYTE_KANJI } from './regex.js';
import { ALPHANUMERIC, BYTE, from, getBestModeForData, getCharCountIndicator, KANJI, NUMERIC, toString } from './mode.js';
...
function buildNodes(segs) {
	...
	for (let i = 0; i < segs.length; i++) {
		...
		switch (seg.mode) {
			...
			case KANJI:
				nodes.push([seg,
					{ data: seg.data, mode: BYTE, length: getStringByteLength(seg.data) }
				]);
				break;
			case BYTE:
				nodes.push([
...
function buildSingleSegment(data, modesHint) {
	...
	if (mode !== BYTE && mode.bit < bestMode.bit) {
      ...
      ` cannot be encoded with mode ${ toString(mode)
